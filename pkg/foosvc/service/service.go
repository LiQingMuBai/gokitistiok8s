...
import (
	...
	addsvcservice "github.com/cage1016/gokitistiok8s/pkg/addsvc/service"
...
func (fo *stubFoosvcService) Foo(ctx context.Context, s string) (res string, err error) {
	res, err = fo.addsvc.Concat(ctx, s, " bar")
	if len(res) >= 104 {
		return "", errors.Wrap(ErrMalformedEntity, errors.New("s length must <= 100"))
	}
	return
}
